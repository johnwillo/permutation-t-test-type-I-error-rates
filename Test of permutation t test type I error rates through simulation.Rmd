...
The for loop below takes the sample sizes and standard deviations in each row of the combos data frame created above, draws two random samples from the same populations, performs a randomization test on each pair of samples and records the p value for the test. It does this the number of times specified in nreps above, and returns the mean proportion of times the p values fell below the alpha level specified (0.05 as entered above). This is the empirical type I error rate for each of the 24 sampling scenarios.

Note that this takes some time to run. The default number of permutations for the perm.t.test() function is 9999 permutations (9999 + the original sample = 10000). I've changed this to 999 given that we are running each of the 24 scenarios and each scenario requires 1000 simulations. Running more simulations than specified in nreps above and more permutations per test would yield more precise estimates of type I error, but will take considerable time to process. The combination of 1000 simulations and 999 permutations per test takes about 22.2 minutes to run. Because we are doing two for loops, one for the normal distribution and one for the skewed normal distribution, this means the entire script takes about 45 minutes to run.

```{r}

for(i in 1:nrow(combos)) {

type.1.err.var.equal = mean(replicate(nreps, 
        perm.t.test(rnorm(combos$n1[i], mean = 0, sd = combos$s1[i]), rnorm(combos$n2[i], mean = 0, sd = combos$s2[i]), alternative = "two.sided", R = 999, var.equal = TRUE)$p.value) < 0.05)

type.1.err.var.unequal = mean(replicate(nreps, 
        perm.t.test(rnorm(combos$n1[i], mean = 0, sd = combos$s1[i]), rnorm(combos$n2[i], mean = 0, sd = combos$s2[i]), alternative = "two.sided", R = 999, var.equal = FALSE)$p.value) < 0.05)

combos[i, 5] = type.1.err.var.equal
combos[i, 6] = type.1.err.var.unequal
...
The combination of 1000 simulations and 1000 permutations per test in the for loop below takes about 22 minutes to run.
...
type.1.err.var.equal = mean(replicate(nreps, 
        perm.t.test(rsnorm(combos2$n1[i], mean = 0, sd = combos2$s1[i]), rnorm(combos2$n2[i], mean = 0, sd = combos2$s2[i]), alternative = "two.sided", R = 999, var.equal = TRUE)$p.value) < 0.05)  

type.1.err.var.unequal = mean(replicate(nreps, 
        perm.t.test(rsnorm(combos2$n1[i], mean = 0, sd = combos2$s1[i]), rnorm(combos2$n2[i], mean = 0, sd = combos2$s2[i]), alternative = "two.sided", R = 999, var.equal = FALSE)$p.value) < 0.05)  

combos2[i, 5] = type.1.err.var.equal
combos2[i, 6] = type.1.err.var.unequal
